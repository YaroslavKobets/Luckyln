...
if (menuLinks.length > 0) {
	menuLinks.forEach((menuLink) => {
		menuLink.addEventListener('click', onMneuLinkClick);
	});

	function onMneuLinkClick(e) {
...
const initForm = () => {
	...
	if (form) {
		...
		let inputMask = new Inputmask({ mask: '+380 (99) 999-99-99', regex: '\\d*' });
		...
		form.addEventListener('submit', (e) => {
			...
			document.querySelector('.modal__first').style.display = 'none';
			document.querySelector('.modal__send').style.display = 'block';
		});
	}
};
initForm();
